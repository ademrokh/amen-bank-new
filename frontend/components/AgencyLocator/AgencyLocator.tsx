...
export default function AgencyLocator() {
  ...
  // Sample branches data - in a real app, this would come from an API
  ...
  const getBranchContent = (branch: Branch) => ({
    name: currentLang === 'fr' ? branch.name_fr : currentLang === 'ar' ? branch.name_ar : branch.name_en,
    city: currentLang === 'fr' ? branch.city_fr : currentLang === 'ar' ? branch.city_ar : branch.city_en,
    address: currentLang === 'fr' ? branch.address_fr : currentLang === 'ar' ? branch.address_ar : branch.address_en,
    hours: currentLang === 'fr' ? branch.hours_fr : currentLang === 'ar' ? branch.hours_ar : branch.hours_en,
  });
  ...
  const filteredBranches = useMemo(() => {
    return branches.filter((branch) => {
      ...
      const content = getBranchContent(branch);
      return (
  ...
  const sectionDescription = currentLang === 'fr'
    ? 'Trouvez l\'agence Amen Bank la plus proche de vous'
    : currentLang === 'ar'
    ? 'جد فرع أمين بنك الأقرب إليك'
    : 'Find the nearest Amen Bank branch to you';

  const searchPlaceholder = currentLang === 'fr'
    ? 'Rechercher par ville ou région...'
    : currentLang === 'ar'
    ? 'ابحث حسب المدينة أو المنطقة...'
    ...
    <section className={`py-20 bg-gradient-to-b from-white to-slate-50 ${isRTL ? 'dir-rtl' : ''}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-4">{sectionTitle}</h1>
          <p className="text-xl text-slate-600 max-w-2xl mx-auto">{sectionDescription}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 lg:row-span-2">
            <div className="bg-gradient-to-br from-slate-200 to-slate-300 rounded-2xl h-96 lg:h-full min-h-96 flex items-center justify-center border border-slate-300 shadow-lg overflow-hidden">
            ...
            {selectedBranch ? (
              <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-lg sticky top-24">
                <h3 className="text-2xl font-bold text-slate-900 mb-4">
                  {getBranchContent(selectedBranch).name}
                </h3>
                ...
                <div className={`flex items-start gap-3 mb-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div>
                    ...
                    <p className="text-slate-900">{getBranchContent(selectedBranch).address}</p>
                  </div>
                </div>
                ...
                <div className={`flex items-start gap-3 mb-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div>
                    <p className="text-sm text-slate-600 font-semibold">
                      {currentLang === 'fr' ? 'Téléphone' : currentLang === 'ar' ? 'الهاتف' : 'Phone'}
                    </p>
                    <a href={`tel:${selectedBranch.phone}`} className="text-blue-900 hover:text-blue-700 font-medium">
                ...
                <div className={`flex items-start gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div>
                    <p className="text-sm text-slate-600 font-semibold mb-2">
                      {currentLang === 'fr' ? 'Horaires' : currentLang === 'ar' ? 'الساعات' : 'Hours'}
                    </p>
                    <p className="text-sm text-slate-700">{getBranchContent(selectedBranch).hours}</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="bg-blue-50 rounded-2xl border border-blue-200 p-6 text-center">
                <p className="text-slate-600">
                  {currentLang === 'fr'
                    ? 'Sélectionnez une agence pour voir les détails'
                    : currentLang === 'ar'
                    ? 'حدد فرعا لعرض التفاصيل'
        ...
        <div className="mt-8">
          <h3 className="text-2xl font-bold text-slate-900 mb-6">
            {currentLang === 'fr'
              ? `${filteredBranches.length} agence${filteredBranches.length !== 1 ? 's' : ''} trouvée${filteredBranches.length !== 1 ? 's' : ''}`
              : currentLang === 'ar'
              ? `${filteredBranches.length} فرع${filteredBranches.length !== 1 ? '' : ''} موجود${filteredBranches.length !== 1 ? '' : ''}`
        ...
        {/* Info Box */}
        <div className="mt-12 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl border border-blue-200 p-8 text-center">
          <p className="text-slate-700 mb-3">
            {currentLang === 'fr'
              ? 'Amen Bank dispose de 164 agences réparties dans 14 régions tunisiennes'
              : currentLang === 'ar'
              ? 'يوجد لدى أمين بنك 164 فرعا موزعة على 14 منطقة تونسية'
              : 'Amen Bank has 164 branches distributed across 14 Tunisian regions'}
          </p>
          <p className="text-sm text-slate-600">
            {currentLang === 'fr'
              ? 'Visitez-nous pour une consultation gratuite avec nos experts'
              : currentLang === 'ar'
              ? 'قم بزيارتنا للحصول على استشارة مجانية من خبرائنا'
              : 'Visit us for a free consultation with our experts'}
          </p>
        </div>
      </div>
    </section>
